...
{
    return;
    // Static critsec destruction may happen after jlog shutdown has happened, and would then crash if we tried to 
    // call DBGLOG
    ...
    {
        DBGLOG("CritSec stats - wait: %s, hold: %s, entered %u times", humanTime(waitCycles).c_str(), humanTime(holdCycles).c_str(), contended+uncontended);
    }
}
