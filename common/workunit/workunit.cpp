...
void CWuGraphStats::beforeDispose()
{
    ...
    IPropertyTree * subgraph = createPTree(tag, progress->queryFlags());
    ...
    ForEach(*iter)
    {
        ...
        if (name.length())
        {
            ...
            if (entry.get())
            {
                ...
                Owned<IPropertyTree> generatedDllBranch = createPTree(generatedDlls->queryFlags());
