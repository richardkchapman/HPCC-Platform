...
public:
    ...
    virtual void         setScheduledOn(char const * name, char const * text) { IPropertyTree * stree = createPTree(tree->queryFlags()); stree->setProp("@name", name); stree->setProp("@text", text); tree->setPropTree("Schedule")->setPropTree("Event", stree); setEnum(tree, "@state", WFStateWait, wfstates); }
