...
IDataVal & CXmlToRawTransformer::transform(IDataVal & result, size32_t len, const void * text, bool isDataSet)
{
    ...
    Owned<IPropertyTree> root = createPTreeFromXMLString(len, (const char *)text, ipt_none|ipt_fast, xmlReadFlags);
...
{
    Owned<IPropertyTree> root = createPTreeFromXMLString(size, utf8, ipt_none|ipt_fast, stripWhitespace ? ptr_ignoreWhiteSpace : ptr_none);
    if (!root)
    {
...
{
    Owned<IPropertyTree> root = createPTreeFromJSONString(size, utf8, ipt_none|ipt_fast, stripWhitespace ? ptr_ignoreWhiteSpace : ptr_none);
    if (!root)
    {
