...
class CReceiveManager : implements IReceiveManager, public CInterface
{
    ...
    public:
        ...
        UdpSenderEntry *nextIndex = nullptr;  // Used to form list of all senders that have outstanding requests
        unsigned timeouts = 0;
        ...
        void requestToSend(unsigned maxTransfer, const IpAddress &returnAddress)
        {
            try
            {
                ...
                msg.cmd = maxTransfer ? flow_t::ok_to_send : flow_t::request_received;
        ...
        virtual int run() 
        {
            ...
            while (running) 
            {
                try 
                {
                    ...
                    update(msg.nodeIp.getNodeAddress(), msg.cmd == sniff_t::busy);
                }
                catch (IException *e) 
                {
    ...
    class receive_receive_flow : public Thread 
    {
        ...
        {
            if ((lastPending == requester) || (requester->nextIndex != nullptr)) // Already on queue
            {
                if (udpTraceLevel > 1)
                {
                    StringBuffer s;
            ...
            else
            {
                ...
                if (pendingRequests != nullptr)
                    lastPending->nextIndex = requester;
                else
        ...
        unsigned sendNextOk()
        {
                    ...
                    else
                    {
                        if (finger != pendingRequests)
                        {
                            if (finger == lastPending)
                                lastPending = prev;
                            assert(prev != nullptr);
                            prev->nextIndex = finger->nextIndex;
                            finger->nextIndex = pendingRequests;
            ...
            if (pendingRequests==lastPending)
                lastPending = nullptr;
            pendingRequests = nextSender->nextIndex;
            nextSender->nextIndex = nullptr;
        ...
        virtual int run() override
        {
            ...
            while (running)
            {
                try
                {
                    ...
                    {
                        StringBuffer ipStr;
                        DBGLOG("UdpReceiver: received %s msg from node=%s", flow_t::name(msg.cmd), msg.sourceNode.getTraceText(ipStr).str());
                    }
