...
private:
    ...
    void sendRequest(flow_t::flowmsg_t cmd, unsigned packets )
    {
        ...
        try
        {
            ...
            {
                StringBuffer s;
                DBGLOG("UdpSender: sending flow_t::%s msg to node=%s", flow_t::name(cmd), ip.getIpText(s).str());
            }
...
public:
    ...
    void sendDone(unsigned packets)
    {
        ...
        if (dataRemaining)
        {
            ...
            sendRequest(flow_t::request_to_send_more, packets);
        }
        else
        {
            requestExpiryTime = 0;
            sendRequest(flow_t::send_completed, packets);
        }
        timeouts = 0;
    }
    ...
    {
        requestExpiryTime = msTick() + udpRequestToSendAckTimeout;
        sendRequest(flow_t::request_to_send, 0);
    }
...
class CSendManager : implements ISendManager, public CInterface
{
    ...
    class send_data : public StartedThread 
    {
        ...
        void send_sniff(sniff_t::sniffmsg_t busy)
        {
