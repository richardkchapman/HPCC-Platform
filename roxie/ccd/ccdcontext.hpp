...
interface IWorkUnitRowReader : public IInterface
{
    ...
    virtual void getResultRowset(size32_t & tcount, const byte * const * & tgt) = 0;
};
...
interface IRoxieServerContext : extends IInterface
{
    ...
    virtual void appendResultDeserialized(const char *name, unsigned sequence, size32_t count, const byte * const *data, bool extend, IOutputMetaData *meta) = 0;
...
{
    virtual int addResult(size32_t count, const byte * const *data, IOutputMetaData *meta) = 0;
    virtual void queryResult(int id, size32_t &count, const byte * const* &data) const = 0;
