...
RoxiePacketHeader::RoxiePacketHeader(const RoxiePacketHeader &source, unsigned _activityId) : serverId(source.serverId)
{
...
{
    numSlaves[channel]++;
    if (node == myNode.getNodeAddress() && channel > 0)
    {
    ...
    if (!localSlave)
    {
        addEndpoint(channel, node, ccdMulticastPort);
    }
}
