...
bool checkEmbeddedFoldable(IHqlExpression* expr, unsigned foldOptions)
{
    ...
    if (body->hasAttribute(_disallowed_Atom) || body->hasAttribute(importAtom) || !body->hasAttribute(foldAtom))
...
IHqlExpression *deserializeConstantSet(ITypeInfo *type, bool isAll, size32_t len, const void *vdata)
{
    ...
    else
    {
        ...
        while (data < end)
        {
            ...
            {
            case type_int:
                if (childType->isSigned())
                    values.append(*createConstant(rtlReadInt(data, size), LINK(childType)));
                else
                    values.append(*createConstant(rtlReadUInt(data, size), LINK(childType)));
                break;
            case type_real:
                if (size==4)
                    values.append(*createConstant(*(float *) data));
                else
                    values.append(*createConstant(*(double *) data));
                break;
            case type_boolean:
                values.append(*createConstant(*(bool *) data));
                break;
            case type_varstring:
...
IHqlExpression * foldEmbeddedCall(IHqlExpression* expr, unsigned foldOptions, ITemplateContext *templateContext)
{
    ...
    IValue *plugin = foldExternalCall(langLoadCall, foldOptions, templateContext);
    if (plugin == nullptr)
        return NULL;

    Owned<IEmbedContext> __plugin = (IEmbedContext *) plugin->getIntValue();  // We declared as int since ecl has no opinter type - not sure what the clean fix is here...
    ...
    for(unsigned i = 0; i < numParam; i++)
    {
        ...
        // if (!foldOptions & HFOpreserveCaseExternalParameter)  // Does not seem to be reliably passed in - is there another way ?
        //    name = str(lower(paramId));
        switch (paramType->getTypeCode())
        {
        case type_int:
        {
