...
IHqlExpression * HqlGram::processEmbedBody(const attribute & errpos, IHqlExpression * embedText, IHqlExpression * language, IHqlExpression *attribs)
{
    ...
    if (language)
    {
        ...
        bool isProjected = hasAttribute(projectedAtom, args);
        if (!isProjected && embedText->queryValue()) // if not constant, they can still put the projected flag on themselves...
        {
            StringBuffer text;
