...
/*
 * Graph progress thoughts:
 * 1. graph state gets 'merged in' to the meta (lightweight) iterator results and is therefore available in the heavy ones too.
 * 2. It's used via both interfaces, but only in the esp cases (both of which iterate in fill).
 * 3. Most calls to getGraphs don't use it, and some calls to getGraphsMeta don't. So don't connect up front.
 * 4. The mechanism is different in the two cases - heavy goes via the wu to retrieve, the lightweight does not. Probably both should.
 * 5. May as well just have a lazy loaded GraphProgress 'branch' (not actually linked to main tree) that is light/heavy loaded just like graphs are.
 * 6. Actually more complex - some people just want graph state. Some just want to SET graph state. some want stats. Some want subgraph level state / update.
 */
